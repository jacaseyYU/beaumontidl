...
pro fade,bub,grs=grs, hcop=hcop
...
if n_params() eq 0 then bubble='36' else bubble=string(bub, format='(i3.3)')
...
cube = cubedir + 'N' + bubble
if keyword_set(hcop) then cube = cube + 'H.fits' else cube = cube + '.fits'
plane=planedir+strtrim(string(bub),2)+'_I4.fits'
