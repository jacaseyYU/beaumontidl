pro updatefield, bubble

;**********************************************************
;interactively determine the JCMT observing window for bubble
;
;UPDATE: May 2008- Use IRAC Fits files- JPEGS don't have consistent
; locations
;**********************************************************
...
infile='/users/cnb/glimpse/fits/i4/'+number+'_I4.fits'
if ~file_test(infile) then stop
im=mrdfits(infile,0,head,/silent)
im=alog(im+1)
nx=n_elements(im[*,0])
ny=n_elements(im[0,*])
