...
if keyword_set(out) then begin
    if ~keyword_set(nosetup) then begin
        ...
        set_plot, 'ps'
        !p.font = 0
        device, /color, bits = 8, file='~/paper/figs/'+string(bubble,format='(i3.3)')+'.ps', $
...
hco = postagestamp(hco, hch, cr, ast.sz[0:1]*ast.cd[0], ast.cd[[0,4]],/nan, npix=ast.sz[0:1])
...
image[0,*,*] = byte(r[bytscl(moment)] * (sum > 0) / max(sum,/nan))
image[1,*,*] = byte(g[bytscl(moment)] * (sum > 0) / max(sum,/nan))
image[2,*,*] = byte(b[bytscl(moment)] * (sum > 0)/ max(sum,/nan))
